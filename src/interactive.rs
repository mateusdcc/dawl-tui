...
fn apply_events(source: &str) -> Result<dawl_tui::DiagramState> {
    let mut state = dawl_tui::DiagramState::default();
    for line in source.lines().filter(|line| !line.trim().is_empty()) { state.apply_json(line)?; }
    Ok(state)
}
