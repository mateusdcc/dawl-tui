...
pub fn text(grid: &Grid) -> String {
    (0..grid.height).map(|y| text_line(grid, y)).collect::<Vec<_>>().join("\n")
}

pub fn ansi(grid: &Grid) -> String {
    let palette = Palette::midnight();
    let mut output = String::new();
    for y in 0..grid.height { write_ansi_line(&mut output, grid, y, palette); }
    output
}
...
pub fn svg(grid: &Grid, cell_width: u16, cell_height: u16) -> String {
    ...
    for y in 0..grid.height { write_svg_row(&mut output, grid, y, cell_width, cell_height, palette); }
    output.push_str("</svg>\n");
    output
}
...
fn text_line(grid: &Grid, y: u16) -> String {
    ...
    for x in 0..grid.width {
        ...
        if !cell.continuation { line.push(grid.visible_char(x, y)); }
    }
...
fn write_ansi_line(output: &mut String, grid: &Grid, y: u16, palette: Palette) {
    ...
    for x in 0..grid.width {
        ...
        if current != Some(cell.style) { write_ansi_style(output, cell.style, palette); }
...
fn write_ansi_style(output: &mut String, style: Style, palette: Palette) {
    ...
    let _ = write!(output, "\u{1b}[38;2;{};{};{}m\u{1b}[48;2;{};{};{}m",
...
fn write_svg_row(output: &mut String, grid: &Grid, y: u16, cell_width: u16, cell_height: u16, palette: Palette) {
    let mut x = 0;
    while x < grid.width {
        ...
        write_svg_run(output, start, y, cell_width, cell_height, style, palette, &content);
    }
}

fn append_svg_cell(content: &mut String, grid: &Grid, x: u16, y: u16) {
    let Some(cell) = grid.cell(x, y) else { return; };
    if !cell.continuation { content.push(grid.visible_char(x, y)); }
}

fn write_svg_run(output: &mut String, x: u16, y: u16, cell_width: u16, cell_height: u16, style: Style, palette: Palette, content: &str) {
