...
fn main() {
    ...
    benchmark("parse", || {
        black_box(parse(SOURCE).expect("parse"));
    });
    benchmark("layout", || {
        black_box(layout_diagram(&diagram, &Default::default()).expect("layout"));
    });
    benchmark("route", || {
        black_box(route_diagram(&diagram, &layout).expect("route"));
    });
    benchmark("render", || {
        black_box(render_diagram(&diagram, &layout, &Default::default()).expect("render"));
    });
...
fn event_repaint(diagram: &dawl_tui::Diagram, layout: &dawl_tui::Layout) {
    ...
    state
        .apply_json_with_graph(EVENT, diagram)
